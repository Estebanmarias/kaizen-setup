    ...
    <section id="reviews" className="py-20 px-6 bg-white">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {REVIEWS.map((r) => (
            <div key={r.title}
              className="bg-gray-50 border border-gray-200 rounded-xl p-6 hover:border-gray-900 transition-colors flex flex-col group">
              <span className="text-xs font-semibold px-2 py-0.5 rounded-full w-fit mb-4 bg-gray-200 text-gray-700">
                {r.tag}
              </span>
              <h3 className="font-semibold text-base mb-2 text-gray-900">{r.title}</h3>
              <p className="text-sm text-gray-500 mb-6 leading-relaxed flex-1">{r.summary}</p>
