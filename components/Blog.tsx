    ...
    <section id="blog" className="py-20 px-6 bg-white dark:bg-[#0f0f0f]">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-end justify-between mb-4">
          ...
          <Link href="/blog" className="text-sm font-semibold text-blue-500 hover:underline hidden sm:block">
            View All Posts →
          </Link>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {posts.map((post) => (
              <a key={post.link} href={post.link} target="_blank" rel="noopener noreferrer"
                ...
                <h3 className="font-semibold text-base mb-2 text-gray-900 dark:text-white group-hover:text-blue-500 transition-colors">
                  {post.title}
                </h3>
