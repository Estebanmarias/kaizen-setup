...
function YoutubeEmbed({ id, title }: { id: string; title: string }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden aspect-video bg-black relative group cursor-pointer"
...
export default function UGC() {
  return (
    <section id="ugc" className="py-20 px-6 bg-gray-50">
      <div className="max-w-6xl mx-auto">
        <p className="text-xs font-semibold tracking-widest uppercase text-blue-500 mb-3">In The Wild</p>
        <div className="flex items-end justify-between mb-4">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900">What Creators Are Saying</h2>
          <Link href="/ugc" className="text-sm font-semibold text-blue-500 hover:underline hidden sm:block">
          ...
          {FEATURED.map((v) => (
            <div key={v.id}>
              ...
              <div className="mt-3">
                ...
                <p className="text-xs text-gray-500">by {v.creator}</p>
              </div>
            </div>
          ))}
        </div>
