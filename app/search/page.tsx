...
export default function SearchPage() {
  ...
  const runSearch = useCallback(async (term: string) => {
    ...
    const [productRes, blogRes] = await Promise.all([
      supabase!
        .from("products")
        .select("id, name, slug, price_naira, image_url, category, in_stock, variants")
        .or(`name.ilike.${pattern},description.ilike.${pattern},category.ilike.${pattern}`)
        .order("name"),
      supabase!
        .from("blog_posts")
        .select("id, title, slug, excerpt, created_at, cover_image")
          ...
          <div className="mb-10">
            <form onSubmit={handleSubmit} className="flex gap-3">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                autoFocus
              />
              <button
                type="submit"
                    ...
                    >
                      <div className="relative aspect-square bg-gray-50 dark:bg-gray-900">
                        {p.image_url ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-gray-300 dark:text-gray-700 text-3xl">📦</div>
                        )}
                        {!p.in_stock && (
                      ...
                      <div className="p-3">
                        <p className="text-sm font-medium text-gray-900 dark:text-white line-clamp-2 leading-snug">{p.name}</p>
              ...
              <div className="flex flex-col gap-4">
                {posts.map(post => (
                  <Link
                    key={post.id}
                    href={`/blog/${post.slug}`}
                    className="flex gap-4 p-4 rounded-xl border border-gray-100 dark:border-gray-800 hover:border-blue-500 transition-colors group"
                  >
                    {post.cover_image && (
                      <div className="relative w-20 h-20 rounded-lg overflow-hidden shrink-0 bg-gray-50 dark:bg-gray-900">
                        <Image src={post.cover_image} alt={post.title} fill className="object-cover" />
                      </div>
                    )}
                    <div className="flex flex-col justify-center min-w-0">
                      <p className="text-sm font-semibold text-gray-900 dark:text-white group-hover:text-blue-500 transition-colors line-clamp-2">
                        {post.title}
                      </p>
                      {post.excerpt && (
