import { Instagram, ArrowLeft, ArrowUpRight } from "lucide-react";
    ...
    <main className="min-h-screen bg-white pt-24 pb-20 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <p className="text-xs font-semibold tracking-widest uppercase text-blue-500">Instagram Reels</p>
        </div>
        <h1 className="text-3xl md:text-5xl font-bold mb-4 text-gray-900">Creator Reviews</h1>
        <p className="text-gray-500 mb-12 max-w-xl">
          Real unboxings, reviews, and tutorials from creators who've used our products.
        </p>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {REELS.map((r) => (
            <a key={r.link} href={r.link} target="_blank" rel="noopener noreferrer"
              className="bg-gray-50 border border-gray-200 rounded-2xl p-5 hover:border-blue-500 hover:shadow-sm transition-all group flex flex-col gap-3">
              ...
              <p className="text-sm font-bold text-gray-900 group-hover:text-blue-500 transition-colors leading-snug flex-1">{r.title}</p>
              <p className="text-xs text-gray-400">by {r.creator}</p>
              <span className="flex items-center gap-1 text-xs font-semibold text-blue-500 mt-auto">
                Watch on Instagram <ArrowUpRight size={11} />
              </span>
            </a>
          ))}
        </div>
