...
function getDefaultCombo(variants: Variant[] | null): Record<string, string> {
  if (!variants) return {};
  const defaults: Record<string, string> = {};
  variants.filter(v => v.name && v.options).forEach(g => { defaults[g.name!] = g.options![0]; });
  return defaults;
}
...
// ── Combo Price Table (for card expand) ───────────────────────────────────────
function ComboPriceTable({ variants }: { variants: Variant[] }) {
  const comboPrices = variants.find(v => v.combo_prices)?.combo_prices;
  const groups = variants.filter(v => v.name && v.options);
  if (!comboPrices || groups.length < 2) return null;
  const [rowG, colG] = groups;
  return (
    <table className="w-full text-xs mt-2 border-collapse">
      <thead>
        <tr>
          <th className="text-left pb-1.5 text-gray-400 font-medium pr-3">{rowG.name}</th>
          {colG.options!.map(opt => (
            <th key={opt} className="pb-1.5 text-gray-400 font-medium text-right">{opt}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {rowG.options!.map(row => (
          <tr key={row} className="border-t border-gray-100 dark:border-gray-800">
            ...
            {colG.options!.map(col => {
              const price = comboPrices[`${row}|${col}`];
              return (
                <td key={col} className="py-1.5 text-right text-gray-900 dark:text-white font-semibold">
...
function QuickAddDrawer({ product, onClose, onAdded }: {
  ...
  const [selectedVariants, setSelectedVariants] = useState<Record<string, string>>(
    isCombo ? getDefaultCombo(product.variants) : {}
  );
  const [quantity, setQuantity] = useState(1);
  const [added, setAdded] = useState(false);

  const comboPrice = isCombo ? getComboPrice(product.variants, selectedVariants) : null;
  ...
  const canAdd = !isCombo || allComboSelected(product.variants, selectedVariants);
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-[#111] h-full flex flex-col shadow-2xl animate-slide-in">
        ...
        <div className="p-5 border-t border-gray-200 dark:border-gray-800 flex flex-col gap-2">
          {isCombo && !canAdd && (
            <p className="text-xs text-center text-gray-400">Select all options to add to cart</p>
          )}
          <button onClick={addToCart} disabled={!canAdd}
            className={`w-full flex items-center justify-center gap-2 py-3 rounded-lg font-semibold text-sm transition-colors ${
              ...
              : canAdd ? "bg-blue-500 hover:bg-blue-400 text-white"
                ...
                <div className="mt-3 flex flex-col gap-2 border-t border-gray-200 dark:border-gray-700 pt-3">
                  ...
                  <div className="flex gap-2 mt-1">
                    <button onClick={() => saveEdit(item)}
                      className="flex-1 bg-blue-500 hover:bg-blue-400 text-white text-xs py-1.5 rounded-lg font-semibold transition-colors">Save</button>
                    <button onClick={() => setEditingId(null)}
                      className="flex-1 border border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-400 text-xs py-1.5 rounded-lg font-semibold transition-colors">Cancel</button>
                  </div>
                </div>
              )}
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-gray-50 dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 rounded-xl p-6 animate-pulse h-72" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map(p => {
              ...
              const perOptionMin = !combo && p.variants?.some(v => v.prices?.length)
                ? Math.min(...(p.variants.flatMap(v => v.prices ?? [])))
                : null;
              ...
              return (
                <div key={p.id}
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed flex-1 mb-3">
                      {p.description}
                    </p>
                    ...
                    {p.price_naira ? (
                      ...
                      <div className="mb-3">
                        <div className="flex items-center justify-between">
                          ...
                          <button onClick={() => togglePrices(p.id)}
                            className="flex items-center gap-1 text-xs text-blue-500 hover:text-blue-400 transition-colors">
                            {expandedPrices[p.id]
                              ? <><ChevronUp size={12} /> Hide</>
                              : <><ChevronDown size={12} /> View all prices</>}
                          </button>
                        </div>
                    ...
                    {p.in_stock ? (
                      ...
                      <button disabled
                        className="w-full flex items-center justify-center gap-2 bg-gray-200 dark:bg-gray-800 text-gray-400 py-2.5 rounded-lg font-semibold text-sm cursor-not-allowed mt-auto">
                        Out of Stock
                      </button>
                    )}
                  </div>
